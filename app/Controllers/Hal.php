...
class Hal extends BaseController
{
	...
	// Constructor to initialize the $db property
	public function __construct()
	{
		// Inisialisasi koneksi database
		$this->db = Database::connect(); // Ensure this is correctly connecting to your database
	}

	public function index()
	{
		if (session(encme('UserID')) == '') {
			return redirect()->to(base_url('hal/masuk'));
		}
	...
	function mt($nm = null, $flt = null)
	{
		if ($nm != null) {
			...
			if (desme($nm) == "tbl_submas" and $flt != null) {
				...
				$dtbl = $this->db->table(desme($nm))->get()->getResult();
			}
