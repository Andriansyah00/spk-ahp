...
?>
				...
				<table class="table table-striped">
					...
					<?php foreach ($fldtbl as $k => $v) {
						...
						if ($k == 'id') {
							...
							$lb = ucwords(str_replace('_', ' ', $k));
						}
						//tgl input
						if ($v->edt == '2') {
							$rcn = date('d-m-Y', $data->$k);
						} else
						if ($v->edt == '4') {
							if ($uslvl == 9) {
								$rcn = desme($data->$k);
	...
	function loadalt($rjk)
	{
		...
		foreach ($alt as $r) {
			...
			foreach ($alt as $r2) {
				...
				if (isset($dt->bobot)) {
					...
					if (isset($dtsubmas1->bobot) and isset($dtsubmas2->bobot)) {
						$bb = $dtsubmas1->bobot / $dtsubmas2->bobot;
					}
					$this->db->table("tbl_bobotalt")->insert(["kriteria" => $rjk, "alternatif1" => $r->id, "alternatif2" => $r2->id, "bobot" => $bb]);
				}
				echo "<td ondblclick='edtnil2(`" . $r->id . "`,`" . $r2->id . "`,this)' class='" . (($r->id == $r2->id) ? 'bg-red' : '') . "'>" . number_format($bb, 2) . "</td>";
		...
		foreach ($alt as $r) {
			echo "<th class='text-red'>" . number_format($tot[$r->id], 2) . "</th>";
		};
		echo "</tr></table>";
	}
	...
	function loadkrta()
{
    ...
    // Create a counter variable to index the criteria starting from 1
    $index = 1;
    foreach ($krta as $r) {
        echo "<th width='$prsn%'>" . $r->nama_kriteria . " (K" . $index . ")</th>";
        $tot[$r->id_kriteria] = 0;
        $index++; // Increment the counter
    }
    echo "</tr>";
    
    $index = 1; // Reset the counter for the row headers
    foreach ($krta as $r) {
        echo "<tr><td>" . $r->nama_kriteria . " (K" . $index . ")</td>";
        foreach ($krta as $r2) {
            ...
            if (isset($dt->bobot)) {
                ...
                $bb = 0;
                if ($r->bobot != 0 and $r2->bobot != 0) {
                    $bb = $r->bobot / $r2->bobot;
                }
                $this->db->table("tbl_bobotk")->insert(["kriteria1" => $r->id_kriteria, "kriteria2" => $r2->id_kriteria, "bobot" => $bb]);
            }
            echo "<td id='pbb" . $r->id_kriteria . $r2->id_kriteria . "' ondblclick='edtnil(`" . $r->id_kriteria . "`,`" . $r2->id_kriteria . "`,this)' class='" . (($r->id_kriteria == $r2->id_kriteria) ? 'bg-red' : '') . "'>" . number_format($bb, 2) . "</td>";
    ...
    // Add a row for totals with red text
    echo "<tr><td><strong><span title='Penjumlah tiap kolom yang bersangkutan'>Jumlah</span></strong></td>";
    foreach ($krta as $r) {
        echo "<td class='text-red'><strong>" . number_format($tot[$r->id_kriteria], 2) . "</strong></td>";
    }
    echo "</tr>";
	...
	function konsistensi($tp = null)
{
    ...
    // Menyiapkan nomor urut manual untuk menampilkan ID kriteria dari K1, K2, ...
    $counter = 1; 
    $kriteria_map = []; // Untuk mapping ID kriteria ke nomor K

    foreach ($krta as $r) {
        $tot[$r->id_kriteria] = 0;
        $kriteria_map[$r->id_kriteria] = 'K' . $counter++; // Mapping ID kriteria ke nomor K
    };
    foreach ($krta as $r) {
        foreach ($krta as $r2) {
            $dt = $this->db->table("tbl_bobotk")->where(["kriteria1" => $r->id_kriteria, "kriteria2" => $r2->id_kriteria])->get()->getRow();
    ...
    if ($n > 2) {
        ...
        if ($tp == 'prts') {
            ...
            echo number_format($rasio, 4);
        }
    } else {
        echo "Jumlah kriteria harus lebih dari 2.!!";
    }
}
	...
	?>
		<div class="modal-header">
			<h4 class="modal-title">Rincian Hasil Alternatif</h4>
		</div>
		<div class="modal-body">
			<table class="table table-sm table-striped">
				...
				<tr>
					<td>Tanggal Input</td>
					<th><?= date('d-m-Y', $alt->tgl_input) ?></th>
				</tr>
				<tr>
					<td>Hasil SPK</td>
	...
	function ctkhsl($no)
	{
		...
		$ttg = 0;
		$hsl = json_decode($dt->hasil);
		$n = 1;
	?>
		<center>
			<h4>HASIL PERHITUNGAN<br><?= strtoupper($this->set->app_name) ?><br>TANGGAL : <?= date('d-m-Y', $dt->tanggal) ?></h4>
		</center>
		...
		<div class="callout callout-warning">
			<h5 class="text-orange"><i class="fas fa-bullhorn"></i> CATATAN :</h5>
			<hr>
			<ol class="pl-3">
