...
<div class="row">
	<div class="col-md-12">
		<div class="card card-red card-outline">
			<div class="card-body">
				<?php if ($uslvl >= 7) { ?><a class="btn btn-danger float-left" href="<?= $brlnk ?>"><i class="fa fa-plus-square"></i> Tambah</a><?php } ?>
					...
					<tbody>
						...
						foreach ($dtbl as $r) {
							...
							if ($v->edt == '7') {
									...
									if ($tabel->nama_tbl == "tbl_alternatif" and $k == "nama") {
										$rcn = "<a href='" . base_url('hal/submas/' . encme($rjk1)) . "'>" . $rcn . "</a>";
									}
									if ($tabel->nama_tbl == "tbl_kriteria" and $k == "nama_kriteria") {
										$rcn = "<a href='" . base_url('hal/mt/' . encme('tbl_subk') . '/' . encme($rjk1)) . "'>" . $rcn . "</a>";
									}
									if ($v->ai == 'Y') {
									} else {
								?>
								...
								<?php if ($tabel->nama_tbl == 'tbl_kriteria') { ?>
									<td><input type="checkbox" name="aktif" id="aktif" onchange="gnti(this,'<?= $rjk1 ?>')" <?php if ($r->aktif == 1) {
																																echo "checked";
																															} ?>></td>
								<?php } ?>
	...
	<div class="modal-dialog modal-sm">
		<div class="modal-content">
			<div class="modal-header">
				<h4><i class="fa fa-question-circle"></i> Konfrmasi..</h4>
			</div>
		...
		$.ajax({
			...
			.done(function(msg) {
				if (msg != "") {
					...
					Toast.fire({
						type: "warning",
						html: "<b>Gagal Memuat Rincian.!!</b><br>Mohon ulangi beberapa saat lagi.!!"
					})
				}
			});
	}
		...
		$.ajax({
			...
			.done(function(msg) {
				if (msg == "Y") {
					Toast.fire({
						type: "success",
						html: "<b>Berhasil Mengubah Data.</b><br>Kriteria berhasil di " + al
					});
				} else {
					Toast.fire({
						type: "warning",
						html: "<b>Gagal Mengubah Data.!!</b><br>Mohon ulangi beberapa saat lagi.!!"
					})
				}
			});
	}
			...
			$.ajax({
				...
				.done(function(msg) {
					if (msg == '1') {
						Toast.fire({
							type: "success",
							html: "<b>Data berhasil dihapus.!!</b><br>Mohon Refresh Halaman.."
						});
					} else {
						Toast.fire({
							type: "warning",
							html: "<b>Data tidak berhasil dihapus.!!</b><br>Dikarenakan masih menjadi referensi di tabel lain.."
						});
					}
				});
		}
