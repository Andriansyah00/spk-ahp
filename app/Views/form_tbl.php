...
?>
<div class="card card-red card-outline" style="width:80%">
	<form class="form-horizontal" action="<?php echo $actionform ?>" method="post" enctype="multipart/form-data">
		<div class="card-header">
			...
			<?php if ($kdrjk == 'baru') {
			} else { ?><input type="text" name="rjk1" value="<?php echo $rjk1 ?>" hidden="hidden"><?php } ?>
		</div>
		<div class="card-body">
			<div class="row">
				<?php
				foreach ($fldtbl as $k => $v) {
					...
					if ($k == 'nik') {
						...
						<div class="form-group <?php if ($v->edt == '6' or $pjg >= 50) {
													echo "col-sm-12";
												} else {
													echo "col-sm-6";
												} ?>" id="dv<?php echo $k ?>">
							...
							<div>
								<?php if ($v->edt == '7') { ?>
									<select class="form-control select2" id="<?php echo $k ?>" name="<?php echo $k ?>" required="required" style="width: 100%;">
										...
										<?php
										if ($nmtbl == 'tbl_pembayaran' and $v->ref->tblr == 'tbl_tp') {
											$dtt = $db->table($v->ref->tblr)->where(['status' => '1'])->get()->getResult();
										} else
						if ($nmtbl == 'tbl_pembayaran' and $v->ref->tblr == 'tbl_siswa') {
											if ($field[$k] != '') {
												$dtt = $db->table('tbl_siswa')->where(['nis' => $field[$k]])->get()->getResult();
											} else {
												$dtt = $db->table('tbl_siswa')->get()->getResult();/*$this->db->query("select nis,nama from tbl_siswa where nis not in (select siswa from tbl_pembayaran where tp=".$tpa->nomor." and jenis!=3)")->result();*/
											}
										} else if ($nmtbl == 'tbl_pelanggaran' and $v->ref->tblr == 'tbl_kelas') {
											$dtt = $db->table($v->ref->tblr)->where(["th_ajaran" => $tpa->nomor])->get()->getResult();
										} else {
											$dtt = $db->table($v->ref->tblr)->get()->getResult();
										}
								...
								<?php } else { ?>
									<input type="<?php echo $tpe ?>" id="<?php echo $k ?>" name="<?php echo $k ?>" class="form-control <?php if ($v->edt == '2') {
																																		echo 'ambltgl';
																																	} ?>" value="<?php echo $valtxt ?>" maxlength="<?php echo $pjg ?>" <?php if ($v->edt != '5' or $kdrjk == 'baru') {
																																																												echo "required";
																																																											}
																																																											if ($k == 'jumlah' or $k == 'harga' or $k == 'nominal' or $k == 'total_bayar') {
																																																												echo " onkeyup='terbilang(this,`lbl" . $k . "`)'";
																																																											} ?> placeholder="<?php echo ucwords(str_replace('_', ' ', $lb)) ?>">
