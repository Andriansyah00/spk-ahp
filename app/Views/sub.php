	...
	<div class="col-md-12">
		<div class="card card-red card-outline">
			<div class="card-header">
				<h3 class="card-title"><i class="fas fa-clipboard-list"></i> Pembobotan Kriteria dan Alternatf</h3>
			</div>
			<div class="card-body">
					...
					<div class="tab-pane fade show active" id="custom-content-below-kriteria" role="tabpanel" aria-labelledby="custom-content-below-kriteria-tab">
						<div class="card card-default card-outline">
							...
							<div class="card-footer">
								<button type="button" class="btn btn-danger" onclick="konsis()">Cek Konsistensi Bobot Kriteria</button>
							</div>
						</div>
					</div>
...
<script>
		...
		$.ajax({
			...
			success: (msg) => {
				...
				if (hsl <= 10) {
					Toast.fire({
						type: "success",
						html: "Bobot kriteria (" + hsl + "%) dapat deterima karena masih dibawah 10%"
					});
				} else {
					Toast.fire({
						type: "warning",
						html: "Bobot kriteria (" + hsl + "%) tidak dapat deterima karena melebihi 10%"
					});
				}
			},
			error: () => {
	...
	function edtnil(k1, k2, id) {
		...
		<?php if ($uslvl >= 7) { ?>
			...
			<?php } else { ?>Toast.fire({
				type: "warning",
				html: "<b>Anda tidak di izinkan melakukan perubahan ini.!!"
			});
		<?php } ?>
	}
	...
	function modKrta(k1, k2, b) {
		...
		$.ajax({
			...
			if (msg == 'Y') {
				...
				Toast.fire({
					type: "success",
					html: "Bobot kriteria berhasil diubah.."
				});
			} else {
				Toast.fire({
					type: "danger",
					html: "Bobot kriteria tidak berhasil diubah.."
				});
			}
		});
	}
		...
		if (kr != '') {
			$.ajax({
				...
				if (msg == 'Y') {
					...
					Toast.fire({
						type: "success",
						html: "Bobot Alternatif berhasil diubah.."
					});
				} else {
					Toast.fire({
						type: "danger",
						html: "Bobot Alternatif tidak berhasil diubah.."
					});
				}
			});
		} else {
			Toast.fire({
				type: "warning",
				html: "Kriteria tidak boleh kosong.!!"
			});
		}
	}
	...
	function resetK() {
		$.ajax({
			...
			if (msg == 'Y') {
				Toast.fire({
					type: "success",
					html: "Kriteria berhasil direset.."
				});
				loadKrta();
			} else {
				Toast.fire({
					type: "warning",
					html: "Kriteria tidak berhasil direset..!!"
				});
			}
		});
	}
		...
		if (kr != "") {
			$.ajax({
				...
				if (msg == 'Y') {
					Toast.fire({
						type: "success",
						html: "Bobot Kriteria berhasil direset.."
					});
					loadAlt(kr);
				} else {
					Toast.fire({
						type: "warning",
						html: "Bobot Kriteria tidak berhasil direset..!!"
					});
				}
			});
		} else {
			Toast.fire({
				type: "warning",
				html: "Mohon pilih kriteria terlebih dahulu..!!"
			});
		}
	}
</script>