...
<div class="tab-content" id="cc-tabContent">
	<div class="tab-pane fade show" id="cc-krta" role="tabpanel" aria-labelledby="cc-krta-tab">
		...
		echo "<div class='card card-red card-outline'><div class='card-header'><h3 class='card-title'>Matriks Perbandingan Kriteria</h3></div><div class='card-body'>";
		echo "<table class='table table-bordered table-striped'><tr><th>Kriteria</th>";

		$counter = 1; // Inisialisasi counter untuk penomoran K1, K2, ...

		foreach ($krta as $r) {
			echo "<th>" . $r->nama_kriteria . " (K" . $counter . ")</th>"; // Menggunakan counter untuk penomoran
			$counter++;
			$tot[$r->id_kriteria] = 0;
		}
		echo "</tr>";

		$counter = 1; // Reset counter untuk penomoran di baris kiri tabel

		foreach ($krta as $r) {
			echo "<tr><td>" . $r->nama_kriteria . " (K" . $counter . ")</td>"; // Menggunakan counter untuk penomoran
			$counter++;
			foreach ($krta as $r2) {
				$dt = $db->table("tbl_bobotk")->where(["kriteria1" => $r->id_kriteria, "kriteria2" => $r2->id_kriteria])->get()->getRow();
				if (isset($dt->bobot)) {
					$bb = $dt->bobot;
					echo "<td>" . number_format($bb, 2) . "</td>";
				} else {
					$bb = 0;
					echo "<td>0</td>";
				}
		...
		foreach ($krta as $r) {
			echo "<th class='text-red'>" . number_format($tot[$r->id_kriteria], 2) . "</th>";
		};
		echo "</tr></table></div></div>";

		//Tabel matriks bobot kriteria
		...
		foreach ($krta as $r) {
			echo "<th>K" . $counter . "</th>"; // Menggunakan counter untuk penomoran
			$counter++;
		}
		...
		foreach ($krta as $r) {
			echo "<tr><td>K" . $counter . "</td>"; // Menggunakan counter untuk penomoran
			$counter++;
			...
			foreach ($krta as $r2) {
				...
				$dt = $db->table("tbl_bobotk")->where(["kriteria1" => $r->id_kriteria, "kriteria2" => $r2->id_kriteria])->get()->getRow();
				if (isset($dt->bobot)) {
					$bb = $dt->bobot / $tot[$r2->id_kriteria];
					echo "<td>" . number_format($bb, 2) . "</td>";
				} else {
					$bb = 0;
					echo "<td>0</td>";
		...
		$counter = 1; // Reset counter untuk penomoran header di tabel sebelah kanan

		foreach ($krta as $r) {
			echo "<th>K" . $counter . "</th>";
			$counter++;
		}
		echo "<th>Jumlah</th></tr>";

		$counter = 1; // Reset counter untuk penomoran di baris kiri tabel sebelah kanan

		foreach ($krta as $r) {
			echo "<tr><td>K" . $counter . "</td>";
			$counter++;
			...
			foreach ($krta as $r2) {
				$dt = $db->table("tbl_bobotk")->where(["kriteria1" => $r->id_kriteria, "kriteria2" => $r2->id_kriteria])->get()->getRow();
				if (isset($dt->bobot)) {
					$bb = $dt->bobot * $prts[$r2->id_kriteria];
					echo "<td>" . number_format($bb, 2) . "</td>";
				} else {
					$bb = 0;
					echo "<td>0</td>";
			...
			echo "<td class='text-red'>" . number_format($jum, 2) . "</td></tr>";
		}
		...
		foreach ($krta as $r) {
			echo number_format($jml[$r->id_kriteria], 2) . "<br>";
		}
		echo "</td><td width='10px' class='br'></td><td width='20px' align='center'>:</td><td width='10px' class='bl'></td><td>";
		foreach ($krta as $r) {
			echo number_format($prts[$r->id_kriteria], 2) . "<br>";
		}
		echo "</td><td width='10px' class='br'></td><td width='20px' align='center'>=</td><td width='10px' class='bl'></td><td>";
		foreach ($krta as $r) {
			echo number_format($jml[$r->id_kriteria] / $prts[$r->id_kriteria], 2) . "<br>";
		}
		...
		$eig = "";
		foreach ($krta as $r) {
			$eig = $eig . number_format($jml[$r->id_kriteria] / $prts[$r->id_kriteria], 2) . " + ";
			$jeig = $jeig + ($jml[$r->id_kriteria] / $prts[$r->id_kriteria]);
		}
		$eigmax = $jeig / $n;
		...
		$rasio = $ci / $ri[$n - 1];
		if ($rasio > 0.1) {
			$kt = "tidak dapat diterima karena lebih besar";
		} else {
			$kt = "dapat diterima karena lebih kecil";
		}
	...
	<div class="tab-pane fade show" id="cc-alt" role="tabpanel" aria-labelledby="cc-alt-tab">
		...
		foreach ($krta as $r) {
			$tot = [];
			echo "<div class='card card-red card-outline'><div class='card-header'><h3 class='card-title'>Matriks Perbandingan Alternatif berdasarkan Kriteria (<b>" . $r->nama_kriteria . "</b>)</h3></div><div class='card-body'><div class='row'>";
			echo "<div class='col-sm-12'><table class='table table-bordered table-striped'><tr><th width='15%'></th>";
			foreach ($alt as $r2) {
				echo "<th width='15%'>" . $r2->nama . "</th>";
				$tot[$r2->id] = 0;
			}
			echo "<th></th></tr>";
			foreach ($alt as $r2) {
				echo "<tr><td>" . $r2->nama . "</td>";
				foreach ($alt as $r3) {
			...
			echo "<tr><th><span title='Penjumlah tiap kolom yang bersangkutan'>Jumlah</span></th>";
			foreach ($alt as $r2) {
				echo "<th class='text-red'>" . number_format($tot[$r2->id], 2) . "</th>";
			};
			echo "</tr></table></div>";
			...
			foreach ($alt as $r2) {
				...
				echo "<th class='text-red'>" . number_format($jum2 / $n2, 2) . "</th></tr>";
			}
			echo "</tr></table></div>";
			echo "</div></div></div>";
		}
	...
	<div class="tab-pane fade show active" id="cc-hsl" role="tabpanel" aria-labelledby="cc-hsl-tab">
		<div class='card card-red card-outline'>
					...
					<div class="col-sm-12">
						<table class="table table-bordered table-striped" border="1" width='100%'>
							<tr>
								...
								$counter = 1; // Inisialisasi counter untuk penomoran K1, K2, ...
								foreach ($krta as $r) {
									echo "<th width='15%'>K" . $counter . " (" . number_format($prts[$r->id_kriteria], 2) . ")</th>";
									$counter++;
								}
								echo "<th></th></tr><tr>";
								foreach ($alt as $r) {
					...
					<div class='col-sm-12'>
						<table class='table table-bordered table-striped' border="1" width='100%'>
							<tr>
								<th width='15%'></th>
								<?php
								$counter = 1; // Reset counter untuk penomoran header K1, K2, ...
								foreach ($krta as $r) {
									echo "<th width='15%'>K" . $counter . "</th>";
									$counter++;
								}
								...
								foreach ($alt as $r) {
									...
									echo "<th class='text-red'>" . number_format($tot, 2) . "</th></tr>";
									$smpl[$r->nama] = $tot;
								}
								arsort($smpl); ?>
						</table>
			...
			<div class="card-footer">
				<button type='button' onclick='cetak()' class='btn bg-gradient-red'><i class='fa fa-print'></i> Cetak Hasil Perhitungan</button>
			</div>
		</div>
	</div>
